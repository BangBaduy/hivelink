...
import { hashOtpCode } from "./otp-security.ts";
...
export class LinkQuotaExceededError extends Error {
  constructor() {
    super("Account link quota reached.");
    this.name = "LinkQuotaExceededError";
  }
}
...
export async function createOtp(email: string, code: string, type: string = "auth"): Promise<OtpRecord> {
  ...
  const codeHash = hashOtpCode(cleanEmail, code, type);
  ...
  if (sql) {
    ...
    const rows = (await sql`
      INSERT INTO otps (email, code_hash, type, expires_at, verified, attempts)
      VALUES (${cleanEmail}, ${codeHash}, ${type}, NOW() + INTERVAL '3 minutes', FALSE, 0)
      RETURNING id, email, code_hash, type, expires_at, verified, attempts, created_at;
    ...
    for (const otp of memoryOtps) {
      if (otp.email === cleanEmail && otp.type === type && !otp.verified) {
        otp.verified = true;
      }
    }
  ...
  if (sql) {
    const rows = (await sql`
      ...
      SET
        ...
        verified = CASE WHEN otp.code_hash = ${codeHash} THEN TRUE ELSE otp.verified END
      FROM candidate
      WHERE otp.id = candidate.id
      RETURNING (otp.code_hash = ${codeHash}) AS matched;
    `) as any[];
    ...
    found.attempts += 1;
    const matched = found.code_hash === codeHash;
...
export async function createShortUrl(
  ...
  maxAccountLinks: number | null = null
): Promise<UrlRecord> {
  const sql = getSql();
  ...
  if (sql) {
          ...
          const result = await sql.transaction(
            [
              sql`
                ...
                SELECT ${originalUrl}, ${shortSlug}, ${userId}
                WHERE (
                  SELECT COUNT(*) FROM urls WHERE user_id = ${userId}
                ) < ${maxAccountLinks}
    ...
    if (rows.length === 0) throw new LinkQuotaExceededError();
    const r = rows[0];
    return {
      id: r.id,
      user_id: r.user_id,
    ...
    if (
      maxAccountLinks &&
      userId &&
      Array.from(memoryUrls.values()).filter((url) => url.user_id === userId).length >=
        maxAccountLinks
    ) {
...
export async function getUserLinks(
  ...
  if (sql) {
    ...
    const list = Array.from(memoryUrls.values()).filter(
...
export async function updateUserLinkDestination(
  id: string,
  userId: string,
  originalUrl: string
  ...
  for (const record of memoryUrls.values()) {
    if (record.id === id && record.user_id === userId) {
