...
async function main() {
  ...
  await sql`ALTER TABLE otps ADD COLUMN IF NOT EXISTS code_hash CHAR(64);`;
  await sql`UPDATE otps SET code_hash = repeat('0', 64), verified = TRUE WHERE code_hash IS NULL;`;
  await sql`ALTER TABLE otps ALTER COLUMN code_hash SET NOT NULL;`;
  await sql`ALTER TABLE otps DROP COLUMN IF EXISTS code;`;
  ...
  await sql`ALTER TABLE otps DROP CONSTRAINT IF EXISTS otps_code_hash_check;`;
  await sql`ALTER TABLE otps ADD CONSTRAINT otps_code_hash_check CHECK (code_hash ~ '^[0-9a-f]{64}$');`;
