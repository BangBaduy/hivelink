...
if (!otpHashSecret || otpHashSecret.length < 32) {
  throw new Error("A secure OTP hashing secret is required.");
}

function hashOtp(email, code, type) {
  return crypto
...
async function debug() {
  ...
  try {
    ...
    const otpCodeHash = hashOtp(testEmail, otpCode, type);
    ...
    const insertedOtp = await sql`
      INSERT INTO otps (email, code_hash, type, expires_at, verified, created_at)
      VALUES (${testEmail}, ${otpCodeHash}, ${type}, ${expiresAt}, FALSE, ${createdAt})
      RETURNING id, email, type, expires_at, created_at;
    `;
