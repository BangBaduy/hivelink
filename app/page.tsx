...
function TurnstileChallenge({
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const scriptId = "hive-turnstile-script";
...
export default function HiveApp() {
  ...
  const [editDestinationError, setEditDestinationError] = useState("");
  ...
  const handleShortenUrl = async (e: React.FormEvent) => {
    ...
    if (captchaSiteKey && !captchaToken) {
      setShortenerError("Please complete the security verification.");
      return;
    }
  ...
  const openDestinationEditor = (item: UrlItem) => {
    setEditingItem(item);
    setEditDestination(item.originalUrl);
    setEditDestinationError("");
  };

  const closeDestinationEditor = () => {
  ...
  const handleUpdateDestination = async (event: React.FormEvent) => {
    event.preventDefault();
    if (!editingItem) return;

    const nextDestination = editDestination.trim();
    if (!nextDestination.startsWith("https://")) {
      setEditDestinationError(
        "Enter a secure destination beginning with https://."
      );
      return;
    }
    if (nextDestination === editingItem.originalUrl) {
      setEditDestinationError("Enter a different destination URL.");
      return;
    }
    ...
    setEditDestinationError("");
    try {
      const res = await fetch(`/api/urls/${editingItem.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ destinationUrl: nextDestination }),
      });
      const data = await res.json();
      if (!res.ok || !data.success || !data.link) {
        setEditDestinationError(
          data.message || "Failed to update the destination."
        );
        return;
      }

      const updatedUrl = data.link.originalUrl as string;
      setUserLinks((links) =>
        links.map((link) =>
      ...
      setAnalyticsItem((item) =>
        item?.id === editingItem.id
          ? { ...item, originalUrl: updatedUrl }
          : item
      );
      setAnalyticsData((analytics) =>
        analytics?.link.id === editingItem.id
          ? {
              ...analytics,
              link: { ...analytics.link, originalUrl: updatedUrl },
            }
          : analytics
      );
      setEditingItem(null);
      setEditDestination("");
      triggerToast("Destination updated. Your short link and QR code stay the same.");
    } catch {
      setEditDestinationError(
        "Unable to update the destination. Check your connection."
      );
    } finally {
      setUpdatingId(null);
    }
  ...
  return (
    <div className="min-h-screen transition-colors duration-300 bg-[#FDFBF7] dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-[#10B981]/20">
        ...
        <section className="text-center space-y-6 pt-4 pb-2">
          ...
          <div className="max-w-2xl mx-auto bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-xl shadow-slate-900/5 transition-all">
            <form onSubmit={handleShortenUrl} className="space-y-4">
              ...
              {captchaSiteKey && (
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-left dark:border-amber-900 dark:bg-amber-950/30">
                  <p className="mb-2 text-xs font-semibold text-amber-900 dark:text-amber-200">
                    Unusual creation activity detected. Complete this check to continue.
                  </p>
                  <TurnstileChallenge
                    key={captchaVersion}
              ...
              {user && (
                <p className="text-left text-[11px] text-slate-500 dark:text-slate-400">
                  Account quota: up to 8 active links. Delete an existing link to free a slot.
                </p>
              )}
            </form>
                          ...
                          <td className="py-3.5 px-4 text-right whitespace-nowrap">
                            <div className="flex items-center justify-end space-x-1.5">
                              ...
                              <button
                                ...
                                className="p-1.5 text-slate-600 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-blue-50 dark:hover:bg-blue-950/40 rounded-lg transition-colors disabled:opacity-40"
                                title="Edit Destination"
                                aria-label={`Edit destination for ${item.shortSlug}`}
                              >
                                {updatingId === item.id ? (
                                  <RefreshCw className="w-4 h-4 animate-spin" />
                                ) : (
      ...
      {/* Edit Destination Confirmation Modal */}
      {editingItem && (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/70 dark:bg-slate-950/85 backdrop-blur-sm"
          role="dialog"
          aria-modal="true"
        ...
        >
          <form
            ...
            className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 border border-blue-200 dark:border-blue-900 shadow-2xl space-y-5"
          >
            <div className="flex items-start gap-3">
              ...
              <div className="pr-8">
                <h3
                  id="edit-destination-title"
                  className="text-lg font-bold text-slate-900 dark:text-white"
                >
                  Update destination
                </h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  The short link, QR code, click total, and analytics history will stay unchanged.
                </p>
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-1">
              <p className="text-xs font-bold text-emerald-700 dark:text-emerald-400">
                /{editingItem.shortSlug}
              </p>
              <p
                className="text-[11px] text-slate-500 dark:text-slate-400 break-all"
                title={editingItem.originalUrl}
              >
                Current: {editingItem.originalUrl}
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  if (editDestinationError) setEditDestinationError("");
                }}
                autoComplete="url"
                autoFocus
                aria-invalid={Boolean(editDestinationError)}
                aria-describedby={
                  editDestinationError ? "edit-destination-error" : undefined
                }
                className="w-full px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
            ...
            <div className="p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 text-[11px] text-amber-800 dark:text-amber-300 flex items-start gap-2">
              ...
              <p>Future visits will immediately redirect to the new destination.</p>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={closeDestinationEditor}
              ...
              <button
                ...
                disabled={
                  ...
                  editDestination.trim() === editingItem.originalUrl
                }
                className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 text-white text-sm font-bold disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {updatingId === editingItem.id ? (
                  <RefreshCw className="w-4 h-4 animate-spin" />
                ) : (
      ...
      {showPrivacyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl relative space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">Effective Date: July 31, 2026</p>
            <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              <p><strong>Abuse Prevention:</strong> When unusual link-creation activity is detected, Cloudflare Turnstile processes limited request and browser signals to distinguish legitimate users from automated abuse. The verification token is validated by our server and is not used as link analytics.</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 border border-slate-200 dark:border-slate-800 shadow-2xl relative space-y-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-3 text-sm text-slate-600 dark:text-slate-300 leading-relaxed">
              ...
              <p><strong>Service Limits:</strong> Each account may maintain up to eight active links. Custom aliases require an authenticated account. Automated or suspicious creation activity may require a security verification.</p>
